...
const expect = ["Comprehensive Evaluation", "Neurological Examination", "Diagnosis & Guidance", "Care Plan & Guidance", "Personalized Treatment Plan"];
const bring = ["Previous medical records or reports (if have)", "List of current medications", "Any imaging reports (MRI, CT scan, etc.)", "Details of symptoms (duration, frequency, triggers)", "Referral (if required by insurance)"];

export default function Contact() {
  return (
    <>
      <SEO title="Contact" description="Get in touch or request an appointment with Dr. Arden." path="/contact" />
      <section className="container-px max-w-3xl mx-auto pt-36 lg:pt-44 text-center">
        <span className="inline-block text-md leading-[24px] md:text-lg font-normal md:leading-[28px] bg-[#eff0eb] rounded-full px-5 py-1.5">Book appointment</span>
        <h1 className="mt-3 text-4xl md:text-6xl md:leading-[70px] lg:text-[80px] lg:leading-[88px] font-medium font-sans">Request your <br className="hidden md:block" /> Appointment Today</h1>
      </section>
      ...
      <section className="container-px max-w-7xl mx-auto mt-10 mb-16 md:mt-16 md:mb-24">
        <div className="bg-[#EFF0EB] rounded-[16px] md:rounded-[24px] p-3 md:p-4 lg:p-4 flex flex-col gap-3 md:gap-4 lg:gap-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4 lg:gap-4">
            {info.map((c) => (
              <div key={c.label} className="bg-[#FEFFFA] rounded-[14px] md:rounded-[20px] p-3 md:p-4 lg:p-6 flex flex-col justify-between">

...
import type { Article } from "@/modules/blog/data/ArticlesDetails";
    ...
    <>
        ...
        >
          <AnimatePresence initial={false}>
            {!isHovered && (
              ...
              >
                <div className="text-sm leading-[20px] md:text-base md:leading-[24px] text-[#4C4D4D]">
                  {article.date}
                </div>
              </motion.div>
            )}
          ...
          <AnimatePresence initial={false}>
            {!isHovered && (
              <motion.div
                ...
                className="pointer-events-auto overflow-hidden shrink-0"
              >
                <p className="text-sm leading-[20px] md:text-base md:leading-[24px] text-[#4C4D4D]">
                  {article.excerpt}
                </p>
              </motion.div>
            )}
      ...
      >
        ...
        <div className="mt-auto pt-12 flex flex-col justify-end">
          ...
          <h3 className="text-[22.1px] leading-[32px] font-medium text-[#111514]">
            {article.title}
          </h3>
...
export default function Articles() {
  ...
  return (
    <>
      <SEO
        title="Articles"
        description="Articles on neurology, brain health, and what to expect during care."
        path="/articles"
      />
      ...
      <section className="container-px xl:max-w-[1200px] mx-auto pb-24 md:pb-32">
        <div className="bg-[#EFF0EB] rounded-[20px] p-4">
          <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            >
              <Img
                src={getArticleImage(0)}
                fallbackSrc={`/images/articles/${featured.slug}.jpg`}
                alt={featured.title}
              ...
              <div className="absolute bottom-6 left-4 lg:bottom-10 lg:left-10 right-4 lg:right-10 text-white z-10 pointer-events-none">
                <div className="text-[14px] md:text-[16px] leading-[24px] mb-2">
                  {featured.date}
                </div>
                <h2 className="text-[22.1px] leading-[28px] md:text-[30px] md:leading-[36px] lg:text-[40px] lg:leading-[44px] font-medium max-w-[600px] mb-3">
                  {featured.title}
                </h2>
                <p className="text-[15px] lg:text-[18px] leading-[24px] md:leading-[28px] max-w-[600px] opacity-90">
                  {featured.excerpt}
                </p>
              </div>
            </Link>

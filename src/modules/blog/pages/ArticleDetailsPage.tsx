...
import { articles } from "@/modules/blog/data/ArticlesDetails";
import { ArrowLeft } from "lucide-react";
import NotFound from "@/modules/core/pages/NotFoundPage";
import { ArticleCard } from "./ArticlesPage";
  ...
  return (
    <>
      ...
      <section className="w-full p-2 sm:p-3 lg:p-4 xl:p-4 flex flex-col">
        <div className="relative w-full rounded-[24px] lg:rounded-[30px] overflow-hidden h-[calc(100vh-1rem)] sm:h-[calc(100vh-1rem)] lg:h-[calc(100vh-2rem)] min-h-[400px]">
          <Img
            src={imagePath}
            alt={a.title}
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 xl:px-4 pb-24">
        ...
        <div className="relative z-10 mt-[-100px] sm:mt-[-140px] md:mt-[-180px] lg:mt-[-220px] bg-[#EFF0EB] rounded-[24px] p-3 lg:p-4 max-w-[1000px] mx-auto flex flex-col gap-4">
          {/* Title & Excerpt Card */}
          <div className="bg-[#FEFFFA] rounded-[20px] p-4 lg:p-8 border border-black/4">
            <Link
              to="/articles"
              className="inline-flex items-center gap-2 text-base font-medium text-[#111514] transition-opacity"
            >
          ...
          {/* Content Card */}
          <div className="bg-[#FEFFFA] rounded-[18px] p-4 lg:p-8 border border-black/4">
            ...
            <div className="space-y-8">
              {a.content.map((sec) => (
                <section key={sec.heading}>
                  <h2 className="text-[20px] md:text-[32px] leading-[28px] md:leading-[40px] font-medium text-[#111514] font-sans mb-4">
                    {sec.heading}
                  </h2>
                  {sec.body && (
                    <p className="text-base md:text-lg leading-[24px] md:leading-[28px] text-[#4C4D4D] font-normal mb-4">
                      {sec.body}
                    </p>
                  )}
                  {sec.bullets && (

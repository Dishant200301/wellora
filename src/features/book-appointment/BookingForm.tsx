...
export default function BookingForm({
  heading = "Schedule a consultation with Dr. Arden for expert diagnosis.",
      ...
      <div className="max-w-[1300px] mx-auto px-4">
          ...
          <>
            ...
            <div className="hidden lg:flex items-start mb-8">
              <div className="w-[40%] shrink-0">
                <span className="text-[16.2px] leading-[28px] text-[#4C4D4D] font-normal font-sans">Book appointment</span>
              </div>
              <h2 className="w-[60%] text-[43.9px] leading-[56px] font-medium text-[#111514] font-sans">{heading}</h2>
            </div>
          </>
        )}
        ...
        <div className="lg:hidden">
          ...
          <motion.div
            ...
            className="w-full rounded-[20px] overflow-hidden h-[220px] mb-6 sm:h-[340px] md:h-[420px] lg:h-[480px] mb-0"
          >
            <Img src="/images/contact/contact.webp" alt="Consultation" className="w-full h-full object-cover" />
          </motion.div>
            ...
            <div className="bg-[#EFF0EB] rounded-[20px] p-5 sm:p-6 md:p-8 flex flex-col">
              ...
              <div className="mb-6 text-center lg:text-left flex flex-col items-center lg:items-start">
                <div className="flex items-center gap-3 mb-2 justify-center lg:justify-start">
                  <div className="w-8 h-8 rounded-full overflow-hidden shrink-0">
                    <Img src="/images/home/doctor_avatar.webp" alt="Dr. Arden" className="w-full h-full rounded-full object-cover" />
                  </div>
                  <div className="text-[16px] md:text-[18px] font-medium text-[#111514] font-sans">Dr. Arden</div>
                </div>
              ...
              <div className="flex flex-col md:grid md:grid-cols-2 gap-4 mb-4">
                <Field label="Full name" error={errors.name?.message}>
                  <input {...register("name")} placeholder="Jane Smith" className="w-full h-12 rounded-[12px] bg-[#FEFFFA] px-4 text-sm text-[#4C4D4D] font-sans focus:outline-none focus:ring-2 focus:ring-[#1F9B6A]/40" />
                </Field>
                <Field label="Email address" error={errors.email?.message}>
                  <input {...register("email")} placeholder="info@ardenneurology.com" className="w-full h-12 rounded-[12px] bg-[#FEFFFA] px-4 text-sm text-[#4C4D4D] font-sans focus:outline-none focus:ring-2 focus:ring-[#1F9B6A]/40" />
                </Field>
                <Field label="Phone number" error={errors.phone?.message}>
                  <input {...register("phone")} placeholder="+21 3 42 34 56 76" className="w-full h-12 rounded-[12px] bg-[#FEFFFA] px-4 text-sm text-[#4C4D4D] font-sans focus:outline-none focus:ring-2 focus:ring-[#1F9B6A]/40" />
                </Field>
                <Field label="Preferred time" error={errors.time?.message}>
                  <TimeDropdown time={time} openTime={openTime} setOpenTime={setOpenTime} setValue={setValue} />
                </Field>
              </div>

              <div>
                <Field label="Anything you'd like me to know beforehand?" error={errors.message?.message}>
                  <textarea {...register("message")} rows={4} placeholder="Write Your Message" className="w-full h-[120px] rounded-[12px] bg-[#FEFFFA] px-4 py-3 text-sm text-[#4C4D4D] font-sans focus:outline-none focus:ring-2 focus:ring-[#1F9B6A]/40 resize-none" />
                </Field>
              </div>
            </div>
            ...
            >
              <span className="relative block overflow-hidden h-[20px]">
                <span className="flex flex-col transition-transform duration-300 ease-out group-hover:-translate-y-1/2">
                  <span className="h-[20px] flex items-center justify-center">{isSubmitting ? "Sending..." : "Book Appointment"}</span>
                  <span className="h-[20px] flex items-center justify-center">{isSubmitting ? "Sending..." : "Book Appointment"}</span>
                </span>
              </span>
            </button>
            ...
            <div className="bg-[#EFF0EB] p-8 flex flex-col justify-between rounded-[24px]">
              <div className="mb-8">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="text-lg font-medium text-[#111514] font-sans">Dr. Arden</div>
                </div>
                <div className="text-[22.1px] leading-[34px] font-medium text-[#111514] font-sans">Schedule a consultation for expert diagnosis</div>
              </div>
              ...
              <div>
                ...
                <div>
                  <Field label="Anything you'd like me to know beforehand?" error={errors.message?.message}>
                    <textarea {...register("message")} rows={4} placeholder="Write Your Message" className="w-full h-[133px] rounded-[12px] bg-[#FEFFFA] px-4 py-3 text-sm text-[#4C4D4D] font-sans focus:outline-none focus:ring-2 focus:ring-[#1F9B6A]/40 resize-none" />
                  </Field>
                </div>
              </div>
            </div>
        ...
        >
          {times.map((t) => (
            <li key={t}>
              <button
                type="button"
                onClick={() => { setValue("time", t, { shouldValidate: true }); setOpenTime(false); }}
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <label className="block">
      ...
      {children}
      {error && <motion.span initial={{ opacity: 0, y: -3 }} animate={{ opacity: 1, y: 0 }} className="block mt-1 text-xs text-red-600">{error}</motion.span>}
    </label>
  );
}
